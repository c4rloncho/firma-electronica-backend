...
export class DocumentoService {
  ...
  private async saveFile(file: Express.Multer.File, fileName: string): Promise<void> {
      ...
      return await this.entityManager.transaction(async (transactionalEntityManager) => {
        try {
          ...
          if (firmaResult.success) {
            await this.saveSignedFile(firmaResult.signatureInfo.signedFiles[0],document)
  ...
  async getAllDocumentsByRut(
    ...
    let query = this.documentRepository.createQueryBuilder('document')
      .innerJoin('document.signatures', 'signature', 'signature.signerRut = :rut', { rut })
      .select([
        'document.id',
        'document.name',
        'document.fileName',
  ...
  async getInfoDocumentId(id:number){
    return await this.documentRepository.find({where:{id:id},relations:['signatures']})
  }
}